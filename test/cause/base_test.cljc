...
(deftest test-transact
  (testing "new causal base"
    (is (= nil (b/cb->edn (b/new-cb)))))
  (testing "map transactions"
    (let [cb (b/transact- (b/new-cb) [[nil nil {:a 1}]])]
      (is (= {:a 1} (b/cb->edn cb)))
  ...
  (testing "list transactions"
    (let [cb (b/transact- (b/new-cb) [[nil nil [1 2]]])]
      (is (= [1 2] (b/cb->edn cb)))
      ...
      (is (= [\h \i 1 2] (b/cb->edn (b/transact- cb [[(::b/root-uuid cb) c/root-id ["hi"]]]))))
      (is (= [[\h \i] 1 2] (b/cb->edn (b/transact- cb [[(::b/root-uuid cb) c/root-id [["hi"]]]]))))))
  (testing "site-id is shared across nested collections"
    (let [cb (b/transact- (b/new-cb) [[nil nil [:div {:a 1} [:span {:b 2} "abc"]]]])]
      (map (fn [[[_ site-id]]] (is (= site-id (::s/site-id cb))))
           (::b/history cb)))))
